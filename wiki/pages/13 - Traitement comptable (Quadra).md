...
- `vat` = somme des TVA de `net_sales + fees + shipping + adjustments`. Arrondi 2 décimales. Export `.xlsx` (openpyxl).

> [!important] Sortie principale = journal d'écritures
> Ce tableau de synthèse n'est plus la sortie finale : il sert de **vue de contrôle qualité**. Le **livrable Quadra** est désormais le **journal d'écritures en partie double** généré par `backend/journal.py` (`journal_<stem>.xlsx`). Voir [[14 - Journal d'écritures Quadra]] (mapping des comptes, équilibre Débit/Crédit, config).
